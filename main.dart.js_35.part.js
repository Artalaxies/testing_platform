self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var A={kJ:function kJ(){},od:function od(){},
aMH(d,e,f){return new A.RL(f,d,e,null)},
b74(d,e,f,g,h){var x=null,w=B.aa(d).h("af<1,e9>")
return B.fn(B.an(new B.af(d,new A.aCS(!0,e,f,g),w),!0,w.h("ax.E")),x,x,x,x)},
RL:function RL(d,e,f,g){var _=this
_.c=d
_.e=e
_.r=f
_.a=g},
RI:function RI(d,e,f,g){var _=this
_.e=d
_.b=e
_.c=f
_.a=g},
aCS:function aCS(d,e,f,g){var _=this
_.a=d
_.b=e
_.c=f
_.d=g},
aCR:function aCR(d,e){this.a=d
this.b=e},
b8j(d,e,f){var x={},w=y.g
x.a=B.a([new A.jb(d)],w)
if(d.length===0)return B.a([],w)
C.c.a2(e,new A.aDH(x,f))
return x.a},
i8:function i8(){},
iP:function iP(){},
jb:function jb(d){this.a=d},
pd:function pd(){},
aid:function aid(){},
aDH:function aDH(d,e){this.a=d
this.b=e},
PP:function PP(){},
acE:function acE(d,e,f){this.a=d
this.b=e
this.c=f},
C5:function C5(d,e,f){this.d=d
this.b=e
this.a=f},
YH:function YH(){},
atf:function atf(d,e,f){this.a=d
this.b=e
this.c=f},
yJ:function yJ(d,e){this.b=d
this.a=e}},D,C,B,J
a.setFunctionNamesIfNecessary([A])
A=a.updateHolder(c[16],A)
D=c[81]
C=c[2]
B=c[0]
J=c[1]
A.kJ.prototype={
tO(d,e,f){if(e)d.a+="\ufffc"},
yf(d){d.push(D.Px)}}
A.od.prototype={
EZ(d,e,f,g){var x,w=this.a,v=w!=null
if(v)e.mK(0,w.Ak(g))
f.toString
x=f[e.gW_()]
w=x.a
e.Rq(0,w.a,w.b,this.b,x.d,x.c,g)
if(v)e.eN(0)},
bO(d){return d.$1(this)},
J6(d,e){var x=e.a
if(d.a===x)return this
e.a=x+1
return null},
Sh(d,e){return null},
bq(d,e){var x,w,v,u,t,s=this
if(s===e)return C.cK
if(B.H(e)!==B.H(s))return C.bB
x=s.a
w=x==null
v=e.a
if(w!==(v==null))return C.bB
y.h.a(e)
if(!s.e.rB(0,e.e)||s.b!==e.b)return C.bB
if(!w){v.toString
u=x.bq(0,v)
t=u.a>0?u:C.cK
if(t===C.bB)return t}else t=C.cK
return t},
k(d,e){var x=this
if(e==null)return!1
if(x===e)return!0
if(J.a6(e)!==B.H(x))return!1
if(!x.Kq(0,e))return!1
return e instanceof A.od&&e.e.rB(0,x.e)&&e.b===x.b&&!0},
gu(d){var x=this
return B.a4(B.e9.prototype.gu.call(x,x),x.e,x.b,x.c,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a)}}
A.RL.prototype={
N(d,e){var x,w=this,v=null,u=A.b8j(w.c,D.V8,D.HB),t=B.N(e).E.z
t=t==null?v:t.b5(0,w.r)
x=B.N(e).E.z
x=x==null?v:x.b5(0,w.r).aj2(D.a3U,C.hS).b5(0,v)
return B.aH6(A.b74(u,x,w.e,t,!0),v,v,C.bF,!0,v,v,C.aY,v,v,1,C.aQ)}}
A.RI.prototype={}
A.i8.prototype={
k(d,e){if(e==null)return!1
return this.jo(e)},
jo(d){return d instanceof A.i8&&d.a===this.a}}
A.iP.prototype={
k(d,e){if(e==null)return!1
return this.jo(e)},
jo(d){return d instanceof A.iP&&this.Bi(d)&&d.b===this.b}}
A.jb.prototype={
j(d){return"TextElement: '"+this.a+"'"},
k(d,e){if(e==null)return!1
return e instanceof A.jb&&this.Bi(e)},
jo(d){return d instanceof A.jb&&this.Bi(d)}}
A.pd.prototype={}
A.aid.prototype={}
A.PP.prototype={
zw(d,e,f){var x=B.a([],y.g)
C.c.a2(e,new A.acE(this,x,f))
return x}}
A.C5.prototype={
j(d){return"EmailElement: '"+this.d+"' ("+this.a+")"},
k(d,e){if(e==null)return!1
return this.jo(e)},
jo(d){return d instanceof A.C5&&this.Bh(d)&&d.d===this.d}}
A.YH.prototype={
zw(d,e,f){var x=B.a([],y.g)
C.c.a2(e,new A.atf(this,f,x))
return x}}
A.yJ.prototype={
j(d){return"LinkElement: '"+this.b+"' ("+this.a+")"},
k(d,e){if(e==null)return!1
return e instanceof A.yJ&&this.Bh(e)},
jo(d){return d instanceof A.yJ&&this.Bh(d)}}
var z=a.updateTypes(["~(i8)","e9(i8)","~(pd)"])
A.aCS.prototype={
$1(d){var x,w,v=null
if(d instanceof A.iP){x=d.a
w=B.XW(v)
w.aI=new A.aCR(this.c,d)
return new A.RI(B.pm(B.aH6(B.fn(v,w,v,this.b,x),v,v,v,v,v,v,v,v,v,v,v),C.eI,v,v,v),C.a4S,v,v)}else return B.fn(v,v,v,this.d,d.a)},
$S:z+1}
A.aCR.prototype={
$0(){return this.a.$1(this.b)},
$S:0}
A.aDH.prototype={
$1(d){var x=this.a
x.a=d.zw(0,x.a,this.b)},
$S:z+2}
A.acE.prototype={
$1(d){var x,w,v,u,t,s=this
if(d instanceof A.jb){x=$.aTD()
w=d.a
v=x.eK(w)
if(v==null)s.b.push(d)
else{x=v.b
u=x[0]
u.toString
t=C.b.kC(w,u,"")
w=x[1]
u=w
if((u==null?null:u.length!==0)===!0){w=w
w.toString
s.b.push(new A.jb(w))}x=x[2]
w=x
if((w==null?null:w.length!==0)===!0){x=x
x.toString
x=C.b.kC(x,B.ag("mailto:",!0,!1,!1),"")
w="mailto:"+x
s.b.push(new A.C5(x,w,x))}if(t.length!==0)C.c.M(s.b,s.a.zw(0,B.a([new A.jb(t)],y.g),s.c))}}else s.b.push(d)},
$S:z+0}
A.atf.prototype={
$1(d){var x,w,v,u,t,s,r,q,p,o=this
if(d instanceof A.jb){x=$.aUs()
w=d.a
v=x.eK(w)
if(v==null)o.c.push(d)
else{x=v.b
u=x[0]
u.toString
t=C.b.kC(w,u,"")
w=x[1]
u=w
if((u==null?null:u.length!==0)===!0){w=w
w.toString
o.c.push(new A.jb(w))}x=x[2]
w=x
if((w==null?null:w.length!==0)===!0){s=x
x=s.length-1
w=s[x]
if(w==="."){s=C.b.J(s,0,x)
r="."}else r=null
if(!C.b.bn(s,$.aTZ()))q="http://"+s
else q=s
p=C.b.kC(s,B.ag("https?://",!0,!1,!1),"")
x=o.c
x.push(new A.yJ(q,p))
if(r!=null)x.push(new A.jb(r))}if(t.length!==0)C.c.M(o.c,o.a.zw(0,B.a([new A.jb(t)],y.g),o.b))}}else o.c.push(d)},
$S:z+0};(function aliases(){var x=A.i8.prototype
x.Bi=x.jo
x=A.iP.prototype
x.Bh=x.jo})();(function inheritance(){var x=a.inherit,w=a.inheritMany
x(A.kJ,B.e9)
x(A.od,A.kJ)
x(A.RL,B.aN)
x(A.RI,A.od)
w(B.bD,[A.aCS,A.aDH,A.acE,A.atf])
x(A.aCR,B.e6)
w(B.y,[A.i8,A.pd,A.aid])
w(A.i8,[A.iP,A.jb])
w(A.pd,[A.PP,A.YH])
w(A.iP,[A.C5,A.yJ])})()
B.c5(b.typeUniverse,JSON.parse('{"kJ":{"e9":[]},"od":{"kJ":[],"e9":[]},"RL":{"aN":[],"i":[]},"RI":{"od":[],"kJ":[],"e9":[]},"iP":{"i8":[]},"jb":{"i8":[]},"PP":{"pd":[]},"C5":{"iP":[],"i8":[]},"YH":{"pd":[]},"yJ":{"iP":[],"i8":[]}}'))
var y={g:B.u("k<i8>"),h:B.u("od")};(function constants(){var x=a.makeConstList
D.HB=new A.aid()
D.Px=new B.lI("\ufffc",null,null,!0,!0,C.aq)
D.I5=new A.YH()
D.Hk=new A.PP()
D.V8=B.a(x([D.I5,D.Hk]),B.u("k<pd>"))
D.Lw=new B.j(4286755327)
D.KY=new B.j(4282682111)
D.KH=new B.j(4280908287)
D.KG=new B.j(4280902399)
D.a3H=new B.bn([100,D.Lw,200,D.KY,400,D.KH,700,D.KG],B.u("bn<p,j>"))
D.a3U=new B.S0(D.a3H,4282682111)})();(function lazyInitializers(){var x=a.lazyFinal
x($,"bcY","aTD",()=>B.ag("^(.*?)((mailto:)?[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z][A-Z]+)",!1,!0,!1))
x($,"bef","aUs",()=>B.ag("^(.*?)((?:https?:\\/\\/|www\\.)[^\\s/$.?#].[^\\s]*)",!1,!0,!1))
x($,"bdz","aTZ",()=>B.ag("^(https?:\\/\\/)",!1,!1,!1))})()}
$__dart_deferred_initializers__["BxG0U7+BbaoyGjaoTuvJ0ChLjfg="] = $__dart_deferred_initializers__.current
